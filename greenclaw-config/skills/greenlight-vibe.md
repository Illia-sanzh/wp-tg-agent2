...
Save the HTML from Step 1 to a temporary `.html` file, then run the converter script:

```bash
node /app/config/skills/scripts/convert.js /tmp/input.html -o /tmp/output.txt
```
...
**Usage options:**
```bash
node /app/config/skills/scripts/convert.js /tmp/input.html                # output to stdout
node /app/config/skills/scripts/convert.js /tmp/input.html -o /tmp/output.txt  # output to file
...
If user asks to show dynamic content, you can use dynamic placeholders inside values and in attributes like src, href, alt.

Read `/app/config/skills/greenlight-instructions/dynamic-content.md` for all available dynamic placeholders.

If user asks to show post or custom post type loops, read `/app/config/skills/greenlight-instructions/dynamic-loops.md` for query loop syntax.

## Charts

For chart blocks, read `/app/config/skills/greenlight-instructions/charts.md`.
